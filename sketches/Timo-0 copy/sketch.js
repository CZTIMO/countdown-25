...
const CONFIG = {
  SQUARE_SIZE: 400,
  IMAGE_RATIO: 2, // Ratio hauteur/largeur de l'image (1 = carré, 1.5 = rectangulaire)
  SOUND_DELAY_MS: 50,
  FLEE_DISTANCE: 0.3,
  INTRO_WAIT_MS: 2000,
  INTRO_SCALE_MS: 500,
...
function drawImage(x, y, size) {
  if (scrollImage.complete) {
    // Récupérer le ratio réel de l'image
    const actualRatio = scrollImage.height / scrollImage.width;
    const width = size;
    const height = size * actualRatio;
    ctx.drawImage(scrollImage, x - width / 2, y - height / 2, width, height);
  } else {
    ctx.fillStyle = "white";
